...
def trackerOnRes(a,botcx,botcy):
    k=50
    if((botcx in range((xx[a]-k),(xx[a]+k))and(botcy in range((yy[a]-k)),(yy[a]+k)) :
        return 1
    else:
        return 0
...
def gotores(a,current_angle,botcx,botxy):
    angle_res=aa[a]
    if(trackerOnRes(a,botcx,botcy)==True):
        socket.send("s")
        socket.send("b")
    else:        
        while not (angle_res==current_angle):
            current_angle=trackerAngle()
            if(angle_res>current_angle):
                socket.send("l")
            elif(angle_res>current_angle):
                socket.send("r")
            elif(angle_res==current_angle):
                socket.send("f")
