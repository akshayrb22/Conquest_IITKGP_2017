...
while(True):
    ...
    res2 = cv2.bitwise_and(resized,resized, mask= masky)
    gray = cv2.cvtColor(res2, cv2.COLOR_BGR2GRAY)
    #blurred = cv2.GaussianBlur(gray, (5, 5), 0)
    t = cv2.getTrackbarPos('t','frame')
    e1 = cv2.getTrackbarPos('e1','frame')
    e2 = cv2.getTrackbarPos('e2','frame')
    thresh2 = cv2.threshold(gray, t, 255, cv2.THRESH_BINARY)[1]
    ...
    #cv2.resizeWindow('mask',800,300)
    #cv2.resizeWindow('frae',800,300)
    #cv2.resizeWindow('fme',800,300)
    cv2.imshow('fame',image)
    #cv2.imshow('mask',res)
