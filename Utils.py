...
class Utils(object):
    ...
    def arena_one_sort(checkPointList):
        for j in range(0, len(checkPointList) - 1):
            if ((checkPointList[j].distance > (checkPointList[j + 1].distance) / 2) and
                (checkPointList[j].shape == CheckpointShape.SQUARE and
                 checkPointList[j + 1].shape == CheckpointShape.TRIANGLE)):
